...
interface IVoterEscrowLike {
    // Preferred signature: create_lock_for(to, amount, unlock_time)
    function create_lock_for(address to, uint256 amount, uint256 unlock_time) external returns (uint256 tokenId);
    // If your ve uses Solidly order (value, unlock_time, to), set useSolidlyOrder=true in constructor and this adapter will call that instead.
...
contract TraderRewardsLocker is Ownable, Pausable, ReentrancyGuard {
    ...
    event Claimed(uint256 indexed epoch, address indexed user, uint256 share, uint256 lockedAmount, uint256 unlockTime, uint256 tokenId);
    ...
    function _claimTo(uint256 epoch, address account, address receiver) internal {
        ...
        // align unlock to weeks boundary (same as ve practice)
        uint256 unlockTime = ((block.timestamp + (minLockWeeks * 1 weeks)) / 1 weeks) * 1 weeks;
        // clamp to max
        uint256 maxUnlock = ((block.timestamp + (maxLockWeeks * 1 weeks)) / 1 weeks) * 1 weeks;
    ...
    function emergencyWithdraw(address token, address to, uint256 amount) external onlyOwner {
        if (token == address(XPGN)) {
            XPGN.safeTransfer(to, amount);
        } else {
